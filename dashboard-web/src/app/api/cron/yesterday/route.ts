...
async function handle(req: Request): Promise<Response> {
  ...
  try {
    ...
    await recordHeartbeat('cron_yesterday', 'success').catch(() => {});
    return NextResponse.json({ published: stores.length }, { status: 200 });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    await recordHeartbeat('cron_yesterday', 'transient_error', msg).catch(() => {});
    return NextResponse.json({ error: 'cron-yesterday failed' }, { status: 500 });
  }
}
