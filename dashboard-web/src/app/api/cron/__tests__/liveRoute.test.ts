// Route test for /api/cron/live (Inngest → Vercel Cron + QStash migration,
// Stage 2 Task 2.1). The scheduler route: verify cron secret → for each active
// store, publishJob('/api/worker/live-store', { storeId }). NO IL-time gate —
// the every-10-min cadence is DST-agnostic.
//
...
import { GET, POST } from '../live/route';

function req(): Request {
  return new Request('https://x/api/cron/live', { method: 'POST' });
}
...
describe('GET/POST /api/cron/live', () => {
  ...
  it('publishes one live-store job per active store when the secret is valid', async () => {
    ...
    for (const storeId of ['uzoshop', 'zolplus', 'usmile360']) {
      expect(publishJobMock).toHaveBeenCalledWith('/api/worker/live-store', { storeId });
    }
  });
  ...
  it('writes a SUCCESS heartbeat after a successful fan-out', async () => {
    ...
    await POST(req());
    expect(recordHeartbeatMock).toHaveBeenCalledWith('cron_live', 'success');
  });
  ...
  it('a heartbeat write failure never breaks the cron (non-fatal)', async () => {
    ...
    const res = await POST(req());
    // The real work still succeeded; the heartbeat is best-effort.
    expect(res.status).toBe(200);
  });
});
