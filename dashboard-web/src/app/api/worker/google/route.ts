// QStash worker route: per-store GOOGLE platform refresh (Inngest → Vercel Cron
// + QStash migration, Stage 2 Task 2.4). Replaces the Inngest `google-worker`
// (google/job.requested) function.
//
...
export async function POST(req: Request): Promise<Response> {
  ...
  const lockKey = `google:${data.store_id}:${data.scope}`;
  ...
  try {
    await runGoogleWorkerForJob(data);
