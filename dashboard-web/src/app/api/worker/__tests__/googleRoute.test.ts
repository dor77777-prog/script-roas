// Route test for /api/worker/google (Inngest → Vercel Cron + QStash migration,
// Stage 2 Task 2.4). Mirrors metaRoute.test.ts — verify QStash signature →
// parse { store_id, scope } → acquireJobLock('google:'+store_id+':'+scope) →
// runGoogleWorkerForJob(data) → releaseJobLock in finally.
...
const runGoogleWorkerForJobMock = vi.fn();
vi.mock('@/inngest/functions/googleWorker', () => ({
  runGoogleWorkerForJob: (...args: unknown[]) => runGoogleWorkerForJobMock(...args),
}));

import { POST } from '../google/route';

const JOB = { store_id: 'uzoshop', scope: 'hot_metrics', tick_id: 't1' };

function req(): Request {
  return new Request('https://x/api/worker/google', { method: 'POST', body: '{}' });
}
...
beforeEach(() => {
  ...
  runGoogleWorkerForJobMock.mockReset();
  acquireJobLockMock.mockResolvedValue(true);
  releaseJobLockMock.mockResolvedValue(undefined);
  runGoogleWorkerForJobMock.mockResolvedValue(undefined);
});
...
describe('POST /api/worker/google', () => {
  it('returns 401 and does NOT run when the QStash signature is invalid', async () => {
    ...
    expect(runGoogleWorkerForJobMock).not.toHaveBeenCalled();
  });
  ...
  it('parses the job, acquires the lock, runs the handler once, releases', async () => {
    ...
    expect(acquireJobLockMock).toHaveBeenCalledWith('google:uzoshop:hot_metrics');
    expect(runGoogleWorkerForJobMock).toHaveBeenCalledTimes(1);
    expect(runGoogleWorkerForJobMock.mock.calls[0][0]).toEqual(JOB);
    expect(releaseJobLockMock).toHaveBeenCalledWith('google:uzoshop:hot_metrics');
  });
  ...
  it('releases the lock even if the handler throws', async () => {
    verifyQstashRequestMock.mockResolvedValue({ ok: true, raw: JSON.stringify(JOB) });
    runGoogleWorkerForJobMock.mockRejectedValue(new Error('boom'));
