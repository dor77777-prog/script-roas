// Route test for /api/worker/daily-store (Inngest → Vercel Cron + QStash
// migration, Stage 2 Task 2.2). The worker route: verify the QStash signature
// → parse { storeId } → acquireJobLock('daily:'+storeId) (skip if not acquired)
// → runDailyForStore(storeId, yesterdayJerusalem(), inline-step ctx) →
// releaseJobLock in finally.
//
...
import { POST } from '../daily-store/route';

function req(): Request {
  return new Request('https://x/api/worker/daily-store', {
    method: 'POST',
    body: '{}',
  });
}
...
describe('POST /api/worker/daily-store', () => {
