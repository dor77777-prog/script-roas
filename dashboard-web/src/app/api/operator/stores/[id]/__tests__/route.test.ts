          ...
          eq: (col: string, val: unknown) => ({
            maybeSingle: () => {
              ...
              if (table === 'store_webhooks' && col === 'store_id') {
                return Promise.resolve({
                  data: db.ownShopDomain !== null ? { shop_domain: db.ownShopDomain } : null,
                  error: null,
                });
              }
...
const secret = vi.hoisted(() => ({ shopifyDomain: null as string | null }));
vi.mock('@/lib/storeSecretsReader', async () => {
  ...
  return {
    ...
    getStoreSecret: vi.fn(async (_storeId: string, key: string) =>
      key === 'SHOPIFY_DOMAIN' ? secret.shopifyDomain : null,
    ),
  };
});
