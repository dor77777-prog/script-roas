...
//
// === REVISED for gap-closure 08 (2026-05-21) ===
// Plan 08 switched the store-level gross from `current_total_price` (live,
// refund-reflecting) to `total_price` (immutable) AND dropped the cross-day
...
//   uzoshop      68.80  (cross-day, processed_at == DAY_D)
//   zolplus      65.28  (cross-day, processed_at == DAY_D)
//   usmile360    81.08 + 55.35  (cross-day, refunds[0])
//                       (usmile360 refunds[1] has refund_line_items=[] → 0)
//   sameDayOrder 20.00  (same-day, intra-order — counts here too under
//                        gap-closure 08 because no cross-day filter)
//   customItem    5.00  (null product_id — store-level still counts it,
//                        and it is also surfaced in customItemRefundCad)
const STORE_REFUND_DEDUCTION_TOTAL =
  68.8 + 65.28 + 81.08 + 55.35 + 20.00 + 5.00;
// = 295.51 (was 275.51 under the OLD cross-day-only model)
...
// Same-day gross under gap-closure 08 = sameDayOrder.total_price (NOT
// current_total_price). Plan 08 Task 1 swapped the algorithm to total_price.
const SAME_DAY_GROSS = 50.00; // sameDayOrder.total_price (was 30.00 current_total_price)

// Same-day per-product line gross for prod-Z (used to recover the
...
describe('cross-day refunds', () => {
  it('D-C3 invariant 1: store-level = sum(total_price for D, excl test/voided) - sum(refund_line_items.subtotal for refunds.processed_at=D, across ALL refund_line_items including null product_id) (REVISED gap-closure 08 — total_price + no cross-day filter)', () => {
    const result = computeRevenueWithCrossDayRefunds(fixtureOrders, DAY_D, PROBE_TZ);

    // Expected store net = same-day total_price gross - all refunds processed on D.
    //   = 50.00 - 295.51 = -245.51 (D-D3: negative is allowed, no clamping)
    expect(result.storeNetCad).toBeCloseTo(SAME_DAY_GROSS - STORE_REFUND_DEDUCTION_TOTAL, 2);
  ...
  it('D-C3 invariant 3: cross-path reconciliation — storeRefundDeduction == BY_PID refund portion + customItemRefundCad (REVISED gap-closure 08 — BY_PID includes the same-day double-attribution)', () => {
    ...
    // (the 3 probe-real refunds + sameDayOrder all have concrete pids → contribute 0 here).
