...
**Status filtering fix (2026-07-09):** Originally `postgresReaders.fetchCampaigns` dropped rows if they had ZERO activity AND were not currently active (paused/archived with no spend today). This deleted 30-40 Meta conversions per day because campaigns that WERE active during 1-9 JUL but are NOW paused got filtered out, even if they recorded conversions during the active period. **Root cause:** the filter checked current `effective_status` (today) against per-day rows, killing historical activity. **Fix:** new rule — if a row has ANY activity (`spend > 0 OR impressions > 0 OR conversions > 0`), KEEP it regardless of status. Only drop rows with zero activity AND non-active status (visual noise). **Symptom:** dashboard showed 123 Meta conversions vs 163 actual (40 missing). **Result:** all 163 now visible. Removed date-filtering bypass (reverted earlier incomplete fix) since it wasn't the root cause.
...
- ✅ **Fixed 2026-06-24 (403 rate-limit ≠ auth)**: Meta returns HTTP **403** for rate limits too (code 4 / subcode 1504022 "Application request limit reached", is_transient:true). The classifier had `/\b403\b/` in `META_HARD_AUTH` (hard-auth runs first), so such a blip was mis-classified `meta_hot_metrics_auth` and got the false "refresh the token" advice (operator-reported). Fix: removed `/\b401\b/`+`/\b403\b/` from `META_HARD_AUTH` — Meta auth is now keyed on the error CODES (190/102/460) + token/session messages; bare 401/403 remain only in the generic `AUTH_PATTERNS` fallback, reached ONLY when there is no transient/rate-limit signature. So a 403+is_transient/rate-limit → `transient` (silent); a bare 403 with no transient signal → still `token_failure`; a real code-190 still wins (hard-auth first). Pinned by tests incl. the exact prod body.
- ✅ **Root fix 2026-06-24 (OAuthException wrapper ≠ auth)**: Meta wraps EVERY Graph error in `"type":"OAuthException"`, so the classifier's `/OAuthException/i` auth pattern made *non-auth* codes ride the wrapper into a false `token_failure`. Prod incident: `code 100 / subcode 1504018 "Invalid parameter"` (`is_transient:false`, user_msg "יש לנסות טווח תאריכים קצר יותר" / "try a shorter date range") → alerted `meta_hot_metrics_auth` + "refresh the token". Fix (in `lib/notifications/detectAuthError.ts`): **removed `/OAuthException/i` from the Meta auth determination entirely** — `META_HARD_AUTH` was renamed `META_AUTH` and is now the SOLE Meta auth gate (`isAuthError('meta',…)` is true *iff* `META_AUTH` matches; the Meta branch never reaches the generic `PROVIDER_PATTERNS` loop). Meta `token_failure` ⇔ an explicit auth CODE (190/102/460/467/458/459/463/464) OR an explicit auth MESSAGE (invalid/expired/missing access token, session expired/invalid, unauthorized/forbidden). `transient` ⇔ code 1/2 service-unavailable, rate-limit (4/17/32/613/80004, 429), `is_transient:true`. Everything else — incl. **code 100** and a bare `OAuthException` with no code/message — → `unknown` (neutral, NO token advice). Pinned by tests incl. the exact code-100 prod body. **INVESTIGATED + RESOLVED 2026-06-25 (it was TRANSIENT, not volume):** an empirical probe (direct Graph API with each store's token + the `get_hot_*_ids` RPCs via service-role/PostgREST) DISPROVED the volume theory. uzoshop's hot set is **817 ad + 611 adset IDs** and its single-query Insights call **succeeds** (584/512 rows); zolplus's is only **42 + 42** and also succeeds today. Meta handles 800+ IDs in one `IN[...]` filter fine — so 1504018 on zolplus's 42 was NOT a size limit. It was a **transient** Meta-side condition (compute timeout, or a just-deleted entity briefly inside the 24h `recently-changed` hot window, since aged out); at probe time all 4 stores' Meta freshness (ad/adset/campaign metrics+status) read `success`. A chunking fix was briefly shipped (`a5adcaf..58f5a66d`) then **REVERTED**: it does nothing for zolplus (42 < the 50-ID chunk size → still one request) and would ~15× uzoshop's per-tick Insights sub-requests (2 → ~30), worsening the very app-level rate limit (code 4 `Application request limit reached`, subcode 1504022) that uzoshop already hits on its daily `kpi_daily` fetch. The CORRECT handling is the classifier fix above: code 100 → `unknown` (silent for the 10-min worker); code 4/403 rate-limits → `transient` (silent). A *persisting* transient still surfaces via `data_freshness` + RunsPanel. Lesson: empirically reproduce against prod before "fixing" a transient.
- ✅ **Fixed 2026-07-01 (uzoshop `kpi_daily` stuck transient_error — the account KPI was held hostage by the heavy ad-level fetch)**: `cronDaily.ts`'s `fetch-meta` step ran four Meta fetches under `Promise.all`. uzoshop's heavy daily ad-level fetch (`fetchMetaAdInsights`, 600-800 ads) intermittently returns code 2 "service temporarily unavailable" (subcode 1504044) / code 4 rate-limit; under `Promise.all` that ONE rejection discarded the (correct) account-spend result and wrote a persistent `kpi_daily`/`data_daily` `transient_error` (last_success = never since ~06-20) — even though `data_daily.fb_spend_cad` was correct the whole time (the 10-min hot-path worker fills it). Fix: (1) `Promise.all` → `Promise.allSettled` so a supplementary leg's failure no longer discards the KPI; (2) the KPI leg now uses **`fetchMetaSpendForDayLight`** (`level=account`, ONE row, ~500ms) instead of `fetchMetaSpendForDay` — which was NOT light: it internally paginated `fetchMetaAdSetInsights` and summed, so the step ran the heavy adset fetch TWICE, adding the very BUC/rate-limit pressure that causes the blips. Invariant now: `kpi_daily` transient_error ⇔ the account-level spend fetch itself failed; adset/ad/budgets rejections are supplementary (Sentry-only, `quietWhatsapp:true`; the hot-path worker fills campaigns_daily/ads_daily with ON CONFLICT). Data parity holds (Meta guarantees account total == Σ adset). **Adversarial 5-lens review caught that v1's "account KPI" (`fetchMetaSpendForDay`) was secretly the heavy adset fetch** — corrected to the genuine `level=account` `fetchMetaSpendForDayLight` + removed the duplicate adset roundtrip. Known tradeoff: a chronically-failing ad-level *daily* fetch is now Sentry-only (no `kpi_daily` red) — acceptable because the hot-path `ad_metrics` scope already tracks `ads_daily` and the account KPI stays green.
...
**Phase 5b follow-up — `job_locks` RLS (2026-06-24).** Migration `20260624120000_job_locks_rls.sql`. `job_locks` (added 2026-06-21 by the Inngest→QStash migration `20260621120000`) was created AFTER the Phase 5b lockdown, so it never got RLS enabled → Supabase's advisor flagged it (`rls_disabled_in_public`, "table publicly accessible"). It was the ONLY public table not covered by 5b (verified: 24/25 public tables had RLS; job_locks was the gap). Same treatment as the rest: `ALTER TABLE public.job_locks ENABLE ROW LEVEL SECURITY` (no policies → deny-all to anon/PUBLIC; service_role bypasses) + `REVOKE ALL … FROM anon, PUBLIC`. Low risk — the `acquire_job_lock` RPC + the `/api/cron/*` + `/api/worker/*` writers are all service-role (bypass RLS); touches grants/RLS only, never data. NOTE: §3.x's older "RLS disabled on all 10 tables, intentional" prose predates Phase 5b and is stale — RLS is now ENABLED on every public table.
